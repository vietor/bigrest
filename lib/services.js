...
function render_parameters(parameters) {
    return xutil.mapArray(parameters, function(origin) {
        ...
        if (parameter.word)
            parameter.word = new WordTester();
        if (parameter.digit)
            parameter.digit = new DigitTester();
        if (parameter.regexp)
            parameter.regexp = new RegExTester(parameter.regexp);
...
function validate_parameter(req, parameter) {
    ...
    if (!value && value !== 0) {
        ...
        if (parameter.digit && !parameter.digit.test(value))
            return PARAM_TYPE;
        if (parameter.word && !parameter.word.test(value))
            return PARAM_TYPE;
        if (parameter.regexp && !parameter.regexp.test(value))
            return PARAM_TYPE;
        if (parameter.values) {
            if (!xutil.eachBreakArray(
                    parameter.values,
                    function(val) {
                        return value == val;
                    }))
