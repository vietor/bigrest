    ...
    return xutil.mapArray(parameters, function(origin) {
        ...
        if (origin.range) {
            if (!xutil.isArray(origin.range) || origin.range.length != 2)
                ...
                parameter.range = new RangeTester(origin.range[0], origin.range[1]).test;
        } else {
            has_min = !xutil.isNullOrUndefined(origin.range_min);
            has_max = !xutil.isNullOrUndefined(origin.range_max);
            if (has_min && has_max)
                parameter.range = new RangeTester(origin.range_min, origin.range_max).test;
            else if (has_min)
                parameter.range = new RangeMinTester(origin.range_min).test;
            else if (has_max)
                parameter.range = new RangeMaxTester(origin.range_max).test;
        }
        if (origin.length) {
            if (!xutil.isArray(origin.length) || origin.length.length != 2)
                throw new Error("Wrong parameter: length (" + origin.name + ")");
            else
                parameter.length = new LengthTester(origin.length[0], origin.length[1]).test;
        } else {
            has_min = !xutil.isNullOrUndefined(origin.length_min);
            has_max = !xutil.isNullOrUndefined(origin.length_max);
            if (has_min && has_max)
                parameter.length = new LengthTester(origin.length_min, origin.length_max).test;
            else if (has_min)
                parameter.length = new LengthMinTester(origin.length_min).test;
            else if (has_max)
                parameter.length = new LengthMaxTester(origin.length_max).test;
        }
        if (origin.values) {
...
function validate_parameter(req, parameter) {
    ...
    if (!value && value !== 0) {
        ...
        if (parameter.length && !parameter.length(value))
            return PARAM_LENGTH;
        ...
        if (parameter.values && !parameter.values(value))
            return PARAM_VALUE;
        if (parameter.range && !parameter.range(value))
            return PARAM_VALUE;
        if (parameter.match) {
