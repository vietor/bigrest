...
function mapArrayLike(objs, iteratee) {
    var result = [];
    for (var i = 0, length = objs.length; i < length; i++) {
        result.push(iteratee(objs[i], i, objs));
    }
    return result;
}

function mapArray(obj, iteratee) {
    return mapArrayLike(mkArray(obj), iteratee);
}
...
function eachBreakArrayLike(objs, iteratee) {
    var breaked = false;
    for (var i = 0, length = objs.length; i < length; i++) {
...
function eachBreakArray(obj, iteratee) {
    return eachBreakArrayLike(mkArray(obj), iteratee);
}
...
exports.eachBreakArray = eachBreakArray;
