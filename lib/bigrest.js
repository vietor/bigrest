...
var codetexts = (function() {
    ...
    xutil.eachObject(JSON.parse(jsontext), function(message, code) {
...
function weblike(opts) {
    ...
    app.use(function(req, res, next) {
        ...
        req.param = function(name, defaultValue) {
            ...
            var value = req.__bigrest_params[name];
            if (xutil.isNullOrUndefined(value))
                value = defaultValue;
            return value;
        };
        req.dumpParams = function() {
            return req.__bigrest_params || xutil.objExtend(req.query || {}, req.body || {}, req.params || {});
        };
        ...
        if (fks.length > 0) {
            res.on('finish', function() {
                xutil.eachArray(fks, function(k) {
                    try {
                        fs.unlinkSync(req.files[k].path);
                    } catch (e) {}
                });
            });
        }
    ...
    xutil.eachArray(xutil.parseArray(opts.services), function(dirname) {
        services.setup(app, path.join(opts.basepath, dirname), xutil.objExtend({
            visitor: opts.visitor
        }, opts._privates));
    });
    if (opts.statics || opts.static) {
        xutil.eachArray(xutil.parseArray(opts.statics || opts.static), function(row) {
