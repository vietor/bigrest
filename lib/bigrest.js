...
var text400 = xutil.readTemplateText('400.html');
var text404 = xutil.readTemplateText('404.html');
var text500 = xutil.readTemplateText('500.html');
...
function options(opts) {
    ...
    opts._privates = {
        default_failure: function(req, res, name, type) {
            res.status(400).send(opts.debug ? ("[" + name + ":" + type + "] " + req.path) : text400);
        }
    };
    return opts;
}
...
function weblike(opts) {
    ...
    if (opts.viewer) {
        ...
        var viewCache;
        if (xutil.isNullOrUndefined(opts.viewer.cache))
            viewCache = !opts.debug;
        else
            viewCache = !!opts.viewer.cache;
    ...
    if (opts.session) {
        ...
        var storeNeedReady;
        if (xutil.isNullOrUndefined(opts.session.storeNeedReady))
            storeNeedReady = true;
        else
            storeNeedReady = !!opts.session.storeNeedReady;
        if (storeNeedReady)
