...
function weblike(opts) {
    ...
    if (opts.session) {
        ...
        if (xutil.optBoolean(opts.session.storeStatusCheck, true)) {
    ...
    else if(opts.cookieSession) {
        app.use(cookieSession(xutil.objExtend({
            name: 'SESSIONID'
        }, opts.cookieSession)));
    }
    ...
    if (opts.statics || opts.static) {
        xutil.eachArray(xutil.parseArray(options.statics || opts.static), function(row) {
